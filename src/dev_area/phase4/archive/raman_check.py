...
# ------------------ Model ------------------
def rate_model(T, A, U, R, N, Q, use_orbach=True, use_raman=True, use_qtm=True):
    """
    Forward rate with optional term toggles.
    """
    T_safe = np.maximum(T, 1e-300)
    r = 0.0
    if use_orbach:
        r += 10.0**(-A) * np.exp(-U / np.maximum(T_safe, 1e-12))  # Orbach
    if use_raman:
        r += 10.0**(R)  * (T_safe**N)                              # Raman
    if use_qtm:
        r += 10.0**(Q)                                             # QTM
...
def quantiles_all_T(
    Ts, mu, sigmas, rho_AU, rho_RN, qs, Z, *,
    use_orbach=True, use_raman=True, use_qtm=True
):
    ...
    for i, T in enumerate(Ts):
        r = rate_model(
            T, A_s, U_s, R_s, N_s, Q_s,
            use_orbach=use_orbach, use_raman=use_raman, use_qtm=use_qtm
        )
...
def domain_regularizer(mu, compiled, lam_AU, lam_RN, lam_Q, eps=1e-6):
    """ Pull (A,U) toward AU-window compilation, (R,N) toward RN, Q toward Q-window. """
    A,U,R,N,Q = mu
    pen = 0.0
    if lam_AU > 0.0:
        tgt, sd = compiled["AU"]["mean"], np.maximum(compiled["AU"]["sd"], eps)
        pen += lam_AU * ( (A - tgt[0])**2 / (sd[0]**2) + (U - tgt[1])**2 / (sd[1]**2) )
    if lam_RN > 0.0:
...
def objective_global(
    x, T_all, tau_mu_all, alpha_all, qs, Z,
    ...
    lnq_hat = quantiles_all_T(
        T_use, mu, sigmas, rho_AU, rho_RN, qs, Z,
        use_orbach=use_orbach, use_raman=use_raman, use_qtm=use_qtm
    )
    ...
    # Domain regularizers (compile by windows)
...
def main():
    ...
    ap.add_argument("--AU_window", default="", help="Tmin,Tmax for Orbach (A,U) compilation, e.g. '45,58'")
    ap.add_argument("--RN_window", default="", help="Tmin,Tmax for Raman (R,N) compilation")
    ap.add_argument("--Q_window",  default="", help="Tmin,Tmax for QTM (Q) compilation")
    ...
    # -------- NEW: term toggles for testing --------
    ap.add_argument("--zero_orbach", action="store_true",
                    help="Set Orbach term to zero inside the model (testing).")
    ap.add_argument("--zero_qtm", action="store_true",
                    help="Set QTM term to zero inside the model (testing).")
    ...
    # set term toggles (Raman stays on for tests)
    use_orbach = not args.zero_orbach
    use_qtm    = not args.zero_qtm
    use_raman  = True
    ...
    out_row = pd.DataFrame([{
        ...
        "zero_orbach": bool(args.zero_orbach),
        "zero_qtm": bool(args.zero_qtm)
    }])
    ...
    print(f"Term toggles: Orbach={'ON' if use_orbach else 'OFF'} | Raman={'ON' if use_raman else 'OFF'} | QTM={'ON' if use_qtm else 'OFF'}")
