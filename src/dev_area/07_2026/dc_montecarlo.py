...
# ---------------------------------------------------------------------------
# Exact SEF quantiles (preferred targets when tau_star/beta are available)
# ---------------------------------------------------------------------------
...
def sef_ln_quantiles(tau_star, beta, qs):
    ...
    Preferred over the two-piece-normal targets: the TPN is a summary and
    cannot hold the SEF's log-skewness for any beta < 1, so it underweights
    the heavy short-tau tail that the 0.02 quantile probes."""
    b = float(beta)
    dist = levy_stable(b, 1.0, loc=0.0,
                       scale=np.cos(np.pi * b / 2.0) ** (1.0 / b))
...
def draw_params_correlated(mu, sigmas, rho_AU, rho_RN, Z):
    ...
    # clip instead of silently laundering |rho| > 1 through max(1-rho^2, eps)
    rho_AU = float(np.clip(rho_AU, -0.999, 0.999))
    rho_RN = float(np.clip(rho_RN, -0.999, 0.999))
    L_AU = np.array([[sA, 0.0],
                     [rho_AU * sU, sU * np.sqrt(1.0 - rho_AU ** 2)]])
    L_RN = np.array([[sR, 0.0],
                     [rho_RN * sN, sN * np.sqrt(1.0 - rho_RN ** 2)]])
    eps_AU = Z[:, :2] @ L_AU.T
    eps_RN = Z[:, 2:4] @ L_RN.T
    eps_Q  = Z[:, 4] * sQ
...
def fit_qtm_row(T, mu_ln, sigma1_ln, sigma2_ln, ac, Z, qs,
                fix_cross=False, target_lnq=None):
    """
    Fit Q and sQ.  Cross terms sampled from priors (or dropped if absent).

    ac : dict with optional keys A, Ueff, sA, sU, R, n, sR, sN, Q, sQ,
         rho_AU, rho_RN.  Missing keys mean that term is absent.
    target_lnq : precomputed exact SEF quantile targets (preferred);
         if None, falls back to the two-piece-normal quantiles, with mu_ln
         interpreted as the TPN MODE (from the corrected dc_phase2.py).
    """
    if target_lnq is None:
    ...
    # with the global model and the diagnostic plot; previously the fit fixed
    # them at their means, forcing sQ to absorb cross-term width).
    fixed, cross = {}, {}
    if has_orbach:
        if fix_cross:
            fixed.update({'A': ac['A'], 'Ueff': ac['Ueff']})
        else:
            Ad, Ud = corr_pair_draws(ac['A'], ac['Ueff'],
                                     ac.get('sA', 0.0), ac.get('sU', 0.0),
                                     ac.get('rho_AU', 0.0), Z[:, 0], Z[:, 1])
            cross.update({'A': Ad, 'Ueff': Ud})
    if has_raman:
        if fix_cross:
            fixed.update({'R': ac['R'], 'n': ac['n']})
        else:
            Rd, nd = corr_pair_draws(ac['R'], ac['n'],
                                     ac.get('sR', 0.0), ac.get('sN', 0.0),
                                     ac.get('rho_RN', 0.0), Z[:, 2], Z[:, 3])
    ...
    def objective(x):
        ...
        lnq_hat = simulate_quantiles_reduced(
            T,
            free_param_draws={'Q': Q_draws, **cross},
            fixed_params=fixed,
            qs=qs
        )
...
def fit_raman_row(T, mu_ln, sigma1_ln, sigma2_ln, ac, Z, qs,
                  fix_cross=False, target_lnq=None):
    """
    Fit R, n, sR, sN.  Cross terms sampled from priors (or dropped).
    target_lnq: precomputed exact SEF quantiles (preferred); TPN fallback
    interprets mu_ln as the MODE.
    """
    if target_lnq is None:
        target_lnq = tplognormal_quantiles(mu_ln, sigma1_ln, sigma2_ln, qs)
    ...
    if has_Q:
        if fix_cross:
            ...
            cross.update({'Q': ac['Q'] + Z[:, 4] * ac.get('sQ', 0.0)})
    ...
    def objective(x):
        ...
        lnq_hat = simulate_quantiles_reduced(
            T,
            free_param_draws={'R': R_draws, 'n': n_draws, **cross},
            fixed_params=fixed,
            qs=qs
        )
        resid = lnq_hat - target_lnq
...
def fit_one_row(df, idx, out_path, ac, seed_base=12345, make_plot=False,
    ...
    ac : dict with any subset of keys:
         A, Ueff, sA, sU, R, n, sR, sN, Q, sQ, rho_AU, rho_RN
    ...
    # exact SEF quantile targets whenever (tau_star, beta) are in the CSV
    # (corrected dc_phase2.py writes them); TPN targets only as fallback
    target_lnq = None
    if "tau_star" in row.index and "beta" in row.index and \
            np.isfinite(row["tau_star"]) and np.isfinite(row["beta"]):
        target_lnq = sef_ln_quantiles(float(row["tau_star"]),
                                      float(row["beta"]), qs)

    if in_qtm:
        result = fit_qtm_row(T, mu_ln, sigma1_ln, sigma2_ln, ac, Z, qs,
                             fix_cross=fix_cross, target_lnq=target_lnq)
    else:
...
def main():
    ...
    pr.add_argument("--fix_cross", action="store_true",
                    help="Fix cross terms at their prior MEANS instead of "
                         "sampling them with their prior spreads (old "
                         "behaviour; inconsistent with the diagnostic plot and "
                         "pushes cross-term width into the fitted sigmas)")
    ...
    ac_priors = {'rho_AU': float(np.clip(args.pr_rho_AU, 0.0, 0.999)),
    ...
    if n_raman:
        print("NOTE: per-row (R, n, sR, sN) fitted at a SINGLE temperature are "
              "degenerate —\n"
              "  only R + n*log10(T) (location) and the total width are "
              "identified. Treat\n"
              "  per-row values as anchors only; dc_phase2's slope-based "
              "compiled R/n (across\n"
              "  the window) and the global stage are the meaningful "
              "estimates.\n")
