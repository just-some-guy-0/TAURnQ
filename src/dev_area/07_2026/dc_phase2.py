...
from scipy.stats import levy_stable as _levy_stable
...
def _stable(beta):
    """One-sided stable(beta) with Laplace transform exactly exp(-lambda^beta).

    exp[-(t/tau*)^beta] = E[exp(-t·S/tau*)] with S ~ this distribution, so
    the dimensionless relaxation rate is s = tau*/tau ~ S (Johnston 2006)."""
    return _levy_stable(beta, 1.0, loc=0.0,
                        scale=np.cos(np.pi * beta / 2.0) ** (1.0 / beta))


def sef_rlog(log10_tau_star, x, beta, n_quad=None):
    """
    Exact SEF probability density in log10(tau), evaluated at x = log10(tau).

    With s = tau*/tau the change of variables gives
        p(x) = f_S(s) · s · ln(10).

    CORRECTED 2026-07-27.  The previous implementation had (i) a Jacobian
    error — it multiplied by tau instead of s = tau*/tau, tilting the density
    by a factor ~tau^2 (mode displaced ~+0.8 decades, widths ~3x too small
    at beta~0.5), and (ii) an open-ended rectangle rule that dropped the u=0
    endpoint, giving a constant -du/(2*pi) offset that max(0,·) then clipped,
    truncating the tails to exactly zero; the oscillatory quadrature also
    aliased for tau far from tau*.  scipy's one-sided stable pdf replaces
    the hand-rolled quadrature entirely.  (n_quad kept for signature
    compatibility; unused.)
    """
    s = 10.0 ** (log10_tau_star - x)
    return float(_stable(beta).pdf(s) * s * np.log(10.0))


def build_sef_curve(log10_tau_star, beta, xs, n_quad=None):
    """Evaluate the exact SEF rho_log over an array of log10(tau) values
    (vectorised — one pdf call for the whole grid)."""
    s = 10.0 ** (log10_tau_star - np.asarray(xs, dtype=float))
    return _stable(beta).pdf(s) * s * np.log(10.0)
...
def fit_two_piece_lognormal(log10_tau_star, beta, n_grid=600, x_half_range=None):
    ...
    # --- 4. exact quantiles of the SEF distribution (via stable isf) -------
    # Since s = tau*/tau is decreasing in tau:  P(log10 tau <= x) = SF_S(s(x)),
    # so the q-quantile is  x_q = log10(tau*) - log10(isf(q)).
    # These are exact and should be preferred as fitting targets downstream;
    # the two-piece normal is a summary and underestimates the heavy left
    # tail (a TPN cannot hold the SEF's log-skewness for any beta < 1).
    CL = 0.6826894921370859
    dist = _stable(beta)
    q_arr = np.array([0.5 * (1 - CL), 0.5, 0.5 * (1 + CL)])
    q1587, q50, q8413 = log10_tau_star - np.log10(dist.isf(q_arr))

    # --- 5. convert log10 sigmas → ln sigmas (multiply by ln10) ---
    # NOTE: mu is the MODE of the distribution (junction of the two-piece
    # normal), NOT the mean.  For a TPN, mean = mode + sqrt(2/pi)(s2 - s1).
    # Downstream code must never substitute e^<ln tau> (the mean) for mu.
    LN10 = np.log(10.0)
    return {
        "mu_log10":     mu,
        "sigma1_log10": sigma1,
...
def process_file(infile, outfile, make_plot=False,
    ...
    for idx, row in df.iterrows():
        ...
        rec = {
            ...
            # exact SEF quantiles (preferred fitting targets downstream)
            "q1587_log10":  fit["q1587_log10"],
            "q50_log10":    fit["q50_log10"],
            "q8413_log10":  fit["q8413_log10"],
