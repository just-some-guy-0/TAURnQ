...
# ---------------------------------------------------------------------------
# Generalised Debye helpers (exact closed forms)
# ---------------------------------------------------------------------------

CL68 = 0.6826894921370859   # erf(1/sqrt(2))
...
def fk_ln_quantiles(tau_m, alpha, qs):
    ...
    Replaces the normal approximation with sigma = 1.82*sqrt(a)/(1-a), which
    matched neither the exact SD nor the exact 68% half-width and, having
    Gaussian instead of exponential tails, badly missed the outer quantiles
    (q = 0.98 at alpha = 0.1: exact 2.05 vs normal 1.31, ln units)."""
    a  = float(alpha)
    qs = np.asarray(qs, dtype=float)
    return np.log(tau_m) + (2.0 / (1.0 - a)) * np.arctanh(
        np.tan(a * np.pi / 2.0) * np.tan(np.pi * (1.0 - a) * (qs - 0.5)))


def rho_tau(tau, tau_m, alpha):
    """Exact Generalised Debye density over tau (for the diagnostic plot):

        rho(tau) = (1/(2*pi*tau)) sin(a*pi) /
                   [cosh((1-a) ln(tau/tau_m)) - cos(a*pi)]."""
    a = float(alpha)
    s = np.log(np.asarray(tau, dtype=float) / tau_m)
    return (np.sin(a * np.pi)
            / (2.0 * np.pi * np.asarray(tau, dtype=float)
               * (np.cosh((1.0 - a) * s) - np.cos(a * np.pi))))


def corr_pair_draws(m1, m2, s1, s2, rho, z1, z2):
    """Correlated bivariate normal draws via Cholesky (rho clipped to valid)."""
    rho = float(np.clip(rho, -0.999, 0.999))
    d1 = m1 + z1 * s1
    d2 = m2 + (rho * z1 + np.sqrt(1.0 - rho ** 2) * z2) * s2
    return d1, d2
...
def penalty_RN(R, n, sR, sN):
    ...
    # n < 0 is unphysical for a Raman process — hard wall (weight 1.0), not
    # the soft 1e-3 nudge: single-T rows are degenerate along R + n*log10(T)
    # and a weak wall lets the optimiser park at negative-n ridge points
    if n <   0.0: pen += 1.0  * (0.0 - n) ** 2
...
def fit_orbach_row(T, tau_m, alpha, pr, Z, qs, fix_cross=False):
    """
    Fit A, U, sA, sU. Cross terms (R/n/Q) from priors if available, else drop.

    pr : dict with optional keys R, n, sR, sN, Q, sQ, rho_RN

    Cross terms are SAMPLED with their prior spreads by default — consistent
    with the global model and with the diagnostic plot (which always sampled
    them; the fit previously fixed them at their means, an inconsistency that
    forced the free sigmas to absorb width belonging to the cross terms).
    fix_cross=True restores the old fixed-at-mean behaviour.
    """
    ...
    if has_raman:
        if fix_cross:
            fixed.update({'R': pr['R'], 'n': pr['n']})
        else:
            Rd, nd = corr_pair_draws(pr['R'], pr['n'],
                                     pr.get('sR', 0.0), pr.get('sN', 0.0),
                                     pr.get('rho_RN', 0.0), Z[:, 2], Z[:, 3])
            cross.update({'R': Rd, 'n': nd})
    if has_Q:
        if fix_cross:
            fixed.update({'Q': pr['Q']})
        else:
            cross.update({'Q': pr['Q'] + Z[:, 4] * pr.get('sQ', 0.0)})
...
def fit_raman_row(T, tau_m, alpha, pr, Z, qs, fix_cross=False):
    """
    Fit R, n, sR, sN. Cross terms (A/U/Q) sampled from priors if available,
    else dropped (fix_cross=True fixes them at their means instead).
    """
    target_lnq = fk_ln_quantiles(tau_m, alpha, qs)
    ...
    if has_orbach:
        if fix_cross:
            fixed.update({'A': pr['A'], 'Ueff': pr['Ueff']})
        else:
            Ad, Ud = corr_pair_draws(pr['A'], pr['Ueff'],
                                     pr.get('sA', 0.0), pr.get('sU', 0.0),
                                     pr.get('rho_AU', 0.0), Z[:, 0], Z[:, 1])
            cross.update({'A': Ad, 'Ueff': Ud})
    if has_Q:
        if fix_cross:
            fixed.update({'Q': pr['Q']})
        else:
    ...
    def objective(x):
        ...
        lnq_hat = simulate_quantiles_reduced(
            T,
            free_draws={'R': R_draws, 'n': n_draws, **cross},
            fixed=fixed, qs=qs)
        resid = lnq_hat - target_lnq
...
def fit_qtm_row(T, tau_m, alpha, pr, Z, qs, fix_cross=False):
    """
    Fit Q and sQ. Cross terms (A/U/R/n) sampled from priors if available,
    else dropped (fix_cross=True fixes them at their means instead).
    """
    target_lnq = fk_ln_quantiles(tau_m, alpha, qs)
    ...
    def objective(x):
        ...
        lnq_hat = simulate_quantiles_reduced(
            T,
            free_draws={'Q': Q_draws, **cross},
            fixed=fixed, qs=qs)
        resid = lnq_hat - target_lnq
        return float(np.dot(resid, resid)) + penalty_Q(Q_try, sQ_try)

    res = minimize(objective, x0, method="Nelder-Mead",
                   options={"maxiter": 400, "xatol": 1e-3, "fatol": 2e-4, "disp": False})
...
def fit_one_row(df, idx, out_path, priors, windows, seed_base=12345,
                make_plot=False, fix_cross=False):
    """
    ...
    if in_orbach:
        result = fit_orbach_row(T, tau_m, alpha, priors, Z, qs, fix_cross)
    elif in_raman:
        result = fit_raman_row(T, tau_m, alpha, priors, Z, qs, fix_cross)
    else:
        result = fit_qtm_row(T, tau_m, alpha, priors, Z, qs, fix_cross)
...
def _plot_fit(T, tau_m, alpha, result, idx):
    ...
    ax.plot(taus, rho_fk, color="#D85A30", linewidth=2, label="Generalised Debye target (exact)")
    ax.set_xscale("log")
...
def main():
    ...
    # correlations between cross-term pairs (from ac_phase2 / global stage)
    pr.add_argument("--pr_rho_AU", type=float, default=0.0,
                    help="Correlation of the (A, Ueff) prior pair, in [0, 0.999]")
    pr.add_argument("--pr_rho_RN", type=float, default=0.0,
                    help="Correlation of the (R, n) prior pair, in [-0.999, 0]")
    pr.add_argument("--fix_cross", action="store_true",
                    help="Fix cross terms at their prior MEANS instead of "
                         "sampling them with their prior spreads (restores the "
                         "old behaviour; note it is inconsistent with the "
                         "diagnostic plot and pushes cross-term width into "
                         "the fitted sigmas)")

    args = ap.parse_args()

    # build priors dict — only include keys actually provided
    priors = {'rho_AU': float(np.clip(args.pr_rho_AU, 0.0, 0.999)),
              'rho_RN': float(np.clip(args.pr_rho_RN, -0.999, 0.0))}
    ...
    if n_orbach or n_raman:
        print("NOTE: per-row (mean, sd) pairs fitted at a SINGLE temperature are "
              "degenerate —\n"
              "  only the location and total width of ln(tau) at that T are "
              "identified, not the\n"
              "  individual (A, U, sA, sU) or (R, n, sR, sN). Treat per-row "
              "values as anchors\n"
              "  (the global stage compiles them with medians for this reason) "
              "and rely on the\n"
              "  global fit across the full T range for the decomposition.\n")
